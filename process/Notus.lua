...
if Name ~= 'Notus' then Name = 'Notus' end

if Ticker ~= 'NOT' then Ticker = 'NOT' end
...
Handlers.add(
  ...
  function(m)
      ...
      if validSend(act, m.Tags.Sender) then
        ...
        ao.send({ Target = m.Tags.Sender, Data = "Your Notus balance is now: " .. Balances[m.Tags.Sender] })
      else
        ao.send({Target = m.Tags.Sender, Data = "You can't buy NOtus worth more than 10000 CRED"})
      end
  end
)